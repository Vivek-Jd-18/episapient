...
contract NapaStaking is Ownable{
   ...
   // user structer
    ...
    function stakeTokens(uint256 _amount, uint256 _plan) public {
        ...
        deposit[_msgSender()] = User(_plan, _amount,block.timestamp,block.timestamp+(_plan.mul(1 seconds)), block.timestamp);
    ...
    function _claim()  public  {
        ...
        require(info.amount > 0, "Not Staked");
         ...
         if(block.timestamp+1 seconds> info.endTime){
                ...
                if(info.plan == 30){
                     ratio = apy1.mul(10 ** 18).div(365).div(100);
                    _reward += calculateCompondInterest(info.amount, ratio, timeStaked);
                } else if(info.plan == 60){
                     ratio = apy2.mul(10 ** 18).div(365).div(100);
                    _reward += calculateCompondInterest(info.amount, ratio, timeStaked);
                } else if(info.plan == 90){
    ...
    function calculateCompondInterest(uint256 _principal, uint256 _ratio, uint256 _exponent) internal pure returns (uint256) {
        if (_exponent == 0) {
            return _principal;
        }

        bool isInteger = _ratio.mod(10000000) == 0;
        uint256 accruedReward = ABDKMath64x64.mulu(ABDKMath64x64.pow(ABDKMath64x64.add(ABDKMath64x64.fromUInt(1), ABDKMath64x64.divu(_ratio,10**20)), _exponent), _principal);
        if (isInteger) {
            accruedReward++;
        }
