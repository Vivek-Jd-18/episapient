...
contract Vote {
    ...
    function createPoll(
    ...
    ) public returns (uint[] memory _defaultVotes) {
        ...
        // Set the _defaultVotes output variable to the defaultVotes array
    ...
    function getPollDetails(
        uint8 pollId
    ) public view returns (PollInfo memory pollInfo) {
        ...
        returns (PollVoteOptions memory result, string memory _pollName)
    {
        ...
        require(
            pollDetails[pollId].endTime < block.timestamp,
            "Can't show result now, let Poll end first"
        );
        // Return the poll's vote options and name
