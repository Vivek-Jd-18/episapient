...
contract Vote{
    ...
    function createPoll(address[] memory members,uint256 timeLimit, uint256 voteOptions)public{

        uint256 newPollId = pollCount+1;
        require(pollDetails[newPollId].pollCreator!=msg.sender,"You only can create a single Poll at a time");

        uint[] memory defaultVotes = new uint[](voteOptions);
        
        pollDetails[newPollId] = PollInfo(newPollId,msg.sender,true,members.length,block.timestamp,timeLimit,0,voteOptions,PollVoteOptions(newPollId,defaultVotes));
    ...
    function endPoll(uint8 pollId)public view onlyPollCreator(pollId){
        PollInfo memory poll = pollDetails[pollId];
        poll.pollStatus=false;
    }
    
    function poleResult(uint8 pollId)public view returns(PollVoteOptions memory result){
